...
public class Wisata extends AppCompatActivity implements SearchView.OnQueryTextListener,AdapterView.OnItemClickListener {
    ListView lv;
  ...
  //  private static ArrayAdapter<String> adapter2;
    ...
    private static ListViewAdapter adapter2; //2
    private String [][] coba=new String[1000][3];
    //ArrayAdapter<String> adapter;
    ListViewAdapter adapter;//2
    ArrayAdapter<String> adapter3;//2
    String n="semua";
    String [] nama;//2
    String [] gambar;//2
    String nn,kun,title,kun1;
    int y;//2
    ArrayList<WorldPopulation> arraylist = new ArrayList<WorldPopulation>(); //2
    private String surl = "http://10.0.2.2/wisataCilacap/android/kwisata.php";
    private String url = "http://10.0.2.2/wisataCilacap/android/wisata.php";
    @Override
    ...
    private class CallWebPageTask extends AsyncTask<String, Void, String> {
        ...
        protected Wisata applicationContext;
        @Override
        protected void onPreExecute() {
            this.dialog = ProgressDialog.show(applicationContext, "Loading..", "Akan menampilkan Data Wisata", true);
        }
        @Override
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                JSONArray jsonArray = jsonRootObject.optJSONArray("wisata");
                data=jsonArray;
                daftar = new ArrayList<String>();
                if (n.equals("semua")){
                    for(int i=0; i < data.length(); i++){
                        JSONObject jsonObject = data.getJSONObject(i);
                        daftar.add(jsonObject.optString("namaw").toString());
                        coba[i][0] = jsonObject.optString("namaw").toString();
                        coba[i][1] = jsonObject.optString("linkw").toString();
                        coba[i][2] = jsonObject.optString("ketw").toString();
                        y=y+1;//2
                  ...
                  //  adapter = new ArrayAdapter<String>(getApplicationContext(),R.layout.daftar, R.id.name, daftar);
                    ...
                    aList=arraylist;//2
                    adapter = new ListViewAdapter(Wisata.this, arraylist);//2
                    adapter2=adapter;
                    lv.setAdapter(adapter);
                ...
                if (n.equals("cari")){
                    ...
                    for(int i=0; i < data.length(); i++){
                        JSONObject jsonObject = data.getJSONObject(i);
                        kun=(jsonObject.optString("linkw").toString());
                    }
                    Intent intent = new Intent(Wisata.this, Detailwisata.class);
    ...
    public void readWebpage() {
        if (n.equals("semua")){
            ...
            task.applicationContext = Wisata.this;
            ...
            Toast.makeText(Wisata.this, "Eror " + title, Toast.LENGTH_SHORT).show();
        }
    }
    @Override
